...
switch ($action) {
 ...
 if($imageCheck){
  $message = '<p class="notifyr">An image by that name already exists.</p>';
 } elseif (empty($invId) || empty($imgName)) {
  $message = '<p class="notifyr">You must select a product and image file for the product.</p>';
 } else {
  ...
  if ($result) {
   $message = '<p class="notifyb">The upload succeeded.</p>';
  } else {
   $message = '<p class="notifyr">Sorry, the upload failed.</p>';
  }
 }
...
if ($remove) {
 $message = "<p class='notifyb'>$filename was successfully deleted.</p>";
} else {
 $message = "<p class='notifyr'>$filename was NOT deleted.</p>";
}
...
// Build the image information into HTML for display
    ...
    $imageDisplay = '<p class="notifyr">Sorry, no images could be found.</p>';
    }
