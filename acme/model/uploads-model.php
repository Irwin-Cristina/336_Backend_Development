...
//Obtain Thumbnail images based on invId.
function getallThumbnail($prodId){
 $db = acmeConnect();
 $sql = 'SELECT imgId, imgName, imgPath, inventory.invId, invName FROM images JOIN inventory ON images.invId = inventory.invId WHERE images.invId =:invId AND imgName LIKE \'%-tn.%\'';
 $stmt = $db->prepare($sql);
 $stmt->bindValue(':invId', $prodId, PDO::PARAM_INT);
 $stmt->execute();
 $imageArray = $stmt->fetchAll(PDO::FETCH_ASSOC);
 $stmt->closeCursor();
 return $imageArray;
